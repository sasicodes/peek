...
async fn handle_tunnel_client(socket: WebSocket, registry: Arc<Registry>) {
    ...
    if !registry.validate_token(reg_req.token.as_deref()) {
    ...
    let subdomain = match choose_subdomain(&registry, reg_req.subdomain.as_deref()).await {
        ...
        Err(message) => {
            ...
            send_registration_error(&mut sink, peek_proto::close_codes::AUTH_FAILED, message).await;
            return;
        }
    };
    ...
    if !registry.register(subdomain.clone(), conn.clone()).await {
        ...
        let json = registration_error_json("server at capacity");
...
async fn read_registration(
    ...
    let Ok(Some(Ok(Message::Text(text)))) = timeout else {
...
async fn choose_subdomain(
    ...
    let Some(requested) = requested else {
        return Ok(registry.generate_subdomain().await);
    };

    let Some(requested) = normalize_subdomain(requested) else {
        return Err("invalid subdomain");
    };

    if registry.is_taken(&requested).await {
        Ok(registry.generate_subdomain().await)
    } else {
        Ok(requested)
    }
}
...
async fn send_registration_error(
    ...
    let json = registration_error_json(message);
...
fn registration_error_json(message: &'static str) -> String {
    ...
    serde_json::to_string(&response).unwrap_or_else(|_| {
        r#"{"ok":false,"url":"","subdomain":"","error":"registration failed"}"#.into()
    })
}
...
pub async fn public_handler(
    ...
    let host = request
        ...
        .unwrap_or("")
        .to_string();
    let Some(subdomain) = extract_subdomain(&host, registry.domain()) else {
        return not_found_page();
    };

    let Some(conn) = registry.get(&subdomain).await else {
        return not_found_page();
    };
...
async fn password_gate(
    ...
    let is_auth_post =
        request.method() == axum::http::Method::POST && request.uri().path() == "/__peek_auth";

    if is_auth_post {
        let body_bytes = match request.into_body().collect().await {
            Ok(collected) => collected.to_bytes().to_vec(),
            Err(_) => return Err(password_page(subdomain, Some("Invalid request"))),
        };
        ...
        let submitted_password = form_data
            ...
            .find_map(|pair| {
                ...
                (key == "password")
                    .then(|| urlencoding::decode(val).unwrap_or_default().into_owned())
            })
            .unwrap_or_default();
        ...
        if password_matches {
            ...
            let cookie = format!(
                "peek_auth_{subdomain}={cookie_value}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=86400"
            );
        ...
        return Err(password_page(subdomain, Some("Incorrect password")));
    }
    ...
    if has_valid_cookie {
        ...
        Err(password_page(subdomain, None))
    }
}
